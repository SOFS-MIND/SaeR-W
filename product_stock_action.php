...
if($mode == "add")
{
	        ...
	        global $database, $db;
			$qry_update="INSERT INTO `".TBL_COMMEN_PRODUCT_STOCK."` (`product_name`,`size`,`material`,`qty`,`rate`,`created_by`,`created_dt`,`update_by`,`update_dt`,`status`)"
                        . " VALUES ('".$product_name."','".$size."','".$material."','".$quantity."','".$rate."','".$_SESSION['VFA_username']."',NOW(),'".$_SESSION['VFA_username']."',NOW(),'0');";
			//print_r($qry_update);
			//exit();
            ...
            if($result_upload >0)
            {
				...
				redirect_to('product_stock_management.php');
			}
			else
			{
...
if($mode == "edit")
{
	$id=$_REQUEST['id'];
         global $database, $db;
			$qry_update="UPDATE `".TBL_COMMEN_PRODUCT_STOCK."` SET `product_name`='".$product_name."', `size`='".$size."', `material`='".$material."', `qty`='".$quantity."',`rate`='".$rate."' , `update_by`='".$_SESSION['VFA_username']."', `update_dt`=NOW()
		 WHERE `id`='".$id."' ";
...
if($mode == "required")
{
	...
	if($status != "")
	{
		    ...
		    $result_upload = $database->query( $qry_update );	
				$_SESSION["msg"]="Updated successfully!.";
				redirect_to('product_reqierment_manage.php');
				exit;
			}
			else
			{
	...
	elseif(($quantity > $red_qty) || ($quantity == $red_qty) )
	{
	 ...
	 global $database, $db;
			$qry_update="INSERT INTO `".TBL_REQUIRED."` (`stock_IDFK`,`purpose`,`current_qty`,`required_qty`,`need_qty`,`update_dt`,`status`,`type`)"
                        . " VALUES ('".$stcok_idfk."','".$purpose."','".$quantity."','".$red_qty."','".$need_qty."',NOW(),'".$status."',1);";
			//print_r($qry_update);
			//exit();
			$result_upload = $database->query( $qry_update );
			//echo "The image {$_FILES['media']['name'][$i]} was successfully uploaded and added to the gallery<br />";
